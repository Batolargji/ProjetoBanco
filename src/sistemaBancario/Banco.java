...
class Banco {
    ...
    private Usuario criarUsuario(String tipo, String cpf) {
        ...
        switch (tipo) {
            case "4" :
                novoUsuario = new Dependente("Nome Exemplo", "senha", cpf, "telefone", "email@exemplo.com", "Rua Exemplo", "123", "Bairro Exemplo", "Cidade Exemplo", "UF");
        	case "3":
    ...
    public static String buscarCpfporIDConta(String idConta) {
        ...
        for (File arquivo : pasta.listFiles()) {
            try (BufferedReader br = new BufferedReader(new FileReader(arquivo))) {
                ...
                if (idCorrente.equals(idConta) || idPoupanca.equals(idConta)) {
                    return cpf;
                }
            } catch (IOException e) {
                System.out.println("Erro ao processar o arquivo: " + e.getMessage());
        ...
        if (pasta.exists() && pasta.isDirectory()) {
            for (File arquivo : pasta.listFiles()) {
                try (BufferedReader br = new BufferedReader(new FileReader(arquivo))) {
                    ...
                    String idContaCorrente = "", idContaPoupanca = "";
                    ...
                    while ((linha = br.readLine()) != null) {
                        ...
                        else if (linha.startsWith("ID da Conta Corrente: ")) idContaCorrente = linha.substring(22).trim();
                        else if (linha.startsWith("Saldo da conta corrente: ")) saldoCorrente = Double.parseDouble(linha.substring(25).trim());
                        else if (linha.startsWith("Limite do cheque especial: ")) limiteChequeEspecial = Double.parseDouble(linha.substring(27).trim());
                        else if (linha.startsWith("ID da Conta Poupança: ")) idContaPoupanca = linha.substring(21).trim();
                        else if (linha.startsWith("Saldo da conta poupança: ")) saldoPoupanca = Double.parseDouble(linha.substring(24).trim());
                    }
                    ...
                    if (tipo.equals("1")) {
                        ...
                        usuario = new Dependente(nome, senha, cpf, telefone, email, rua, numero, bairro, cidade, uf);
                    ...
                    if (usuario != null) {
                        ...
                        // Cria as contas associadas e adiciona à lista global
                        if (!idContaCorrente.isEmpty()) {
                            ContaCorrentePrincipal contaCorrente = new ContaCorrentePrincipal(idContaCorrente, saldoCorrente, limiteChequeEspecial);
                            contas.add(contaCorrente);
                        }
                        if (!idContaPoupanca.isEmpty()) {
                            ContaPoupanca contaPoupanca = new ContaPoupanca(idContaPoupanca, saldoPoupanca);
